...
ENCOUNTERS = {
	42 => "O1S", 43 => "O2S", 44 => "O3S", 45 => "Exfaust", 46 => "O4S", 1036 => "Partydad", 1037 => "Snakemom",
...
bot.command :xlogs do |event, *args|
	event << "Quick lookup for *#{$last_charname}* (#{$last_server}):"
...
bot.command :help do |event, *args|
	...
	event << "**!esports** *charactername* *server*: gets data for all current raid content."
	event << "**!bsports** *charactername* *server*: gets data for previous patch raid content."
...
def fflogs_fight(name, server, zone, encounter)
	...
	uri = URI.parse(URI::encode("https://www.fflogs.com:443/v1/parses/character/" + name + "/" + server + "/NA"))
